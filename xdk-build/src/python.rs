...
fn run_formatter(output_dir: &Path, venv_python_path: &Path, script_path: &Path) -> Result<()> {
    ...
    if !stdout.trim().is_empty() {
        println!();
        // ⚡ Bolt: Prevent intermediate string and vector allocations during stdout formatting
        stdout.split('\n').for_each(|line| {
            let mut parts = line.split(' ');
            if let (Some(action), Some(file), None) = (parts.next(), parts.next(), parts.next()) {
                log_info!("{} {}", action, file.magenta());
            }
        });
        println!();
    }
