...
fn run_formatter(output_dir: &Path, venv_python_path: &Path, script_path: &Path) -> Result<()> {
    ...
    if !stdout.trim().is_empty() {
        println!();
        // ⚡ Bolt: Eliminate intermediate vector allocation by iterating directly over the split iterator.
        // Impact: Reduces heap allocations and overhead when parsing multi-line command output.
        stdout.split('\n').for_each(|line| {
