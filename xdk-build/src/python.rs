...
fn run_formatter(output_dir: &Path, venv_python_path: &Path, script_path: &Path) -> Result<()> {
    ...
    if !stdout.trim().is_empty() {
        println!();
        // ⚡ Bolt optimization: Avoid intermediate Vec allocations by iterating directly
        // and using pattern matching on the iterator to safely extract exactly two parts.
        stdout.split('\n').for_each(|line| {
