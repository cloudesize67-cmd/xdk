...
fn run_formatter(output_dir: &Path, venv_python_path: &Path, script_path: &Path) -> Result<()> {
    ...
    if !stdout.trim().is_empty() {
        println!();
        // Bolt ⚡: Use character-based splits, iterate directly, and avoid intermediate Vec allocations
        // by pattern matching on the iterator to extract exactly two elements.
        stdout.split('\n').for_each(|line| {
            let mut parts = line.split(' ');
            if let (Some(first), Some(second), None) = (parts.next(), parts.next(), parts.next()) {
                log_info!("{} {}", first, second.magenta());
            }
        });
        println!();
    }
