...
fn run_formatter(output_dir: &Path, venv_python_path: &Path, script_path: &Path) -> Result<()> {
    ...
    if !stdout.trim().is_empty() {
        println!();
        // Performance optimization: Avoid intermediate Vec allocations
        // Iterate over the Split iterator directly instead of collecting to Vec first
        // Use char-based split ('\n' / ' ') rather than string-based split ("\n" / " ")
        stdout.split('\n').for_each(|line| {
