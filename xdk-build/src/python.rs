...
fn run_formatter(output_dir: &Path, venv_python_path: &Path, script_path: &Path) -> Result<()> {
    ...
    if !stdout.trim().is_empty() {
        println!();
        // OPTIMIZATION: Use character split ('\n') and iterate directly instead of collecting to a Vec
        stdout.split('\n').for_each(|line| {
            // OPTIMIZATION: Use character split (' ') and a mutable iterator to extract elements
