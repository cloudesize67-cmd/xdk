...
pub fn parse_yaml(yaml: &str) -> Result<OpenApi> {
    ...
    if let Some(components) = value.get("components") {
        OpenApiContextGuard::with_context_mut(|ctx| {
            // Performance Optimization: Use `T::deserialize(val)` instead of
            // `from_value(val.clone())` to avoid expensive deep clones of the intermediate
            // YAML value tree. This reduces heap allocations and speeds up parsing.
...
pub fn parse_json(json: &str) -> Result<OpenApi> {
    ...
    if let Some(components) = value.get("components") {
        OpenApiContextGuard::with_context_mut(|ctx| {
            // Performance Optimization: Use `T::deserialize(val)` instead of
            // `from_value(val.clone())` to avoid expensive deep clones of the intermediate
            // JSON value tree. This reduces heap allocations and speeds up parsing.
