...
pub fn parse_yaml(yaml: &str) -> Result<OpenApi> {
    ...
    if let Some(components) = value.get("components") {
        OpenApiContextGuard::with_context_mut(|ctx| {
            ...
            // By utilizing T::deserialize(val) (which accepts &Value since it implements Deserializer)
            // instead of from_value(val.clone()), we completely avoid O(N) deep-cloning of
            // intermediate representation trees (schemas, parameters, responses, etc) which are
            // deeply nested in OpenAPI specs. Expected impact: significantly reduced memory usage
            // and CPU cycles during the pre-loading phase, roughly avoiding 2x memory inflation for parsed objects.
...
pub fn parse_json(json: &str) -> Result<OpenApi> {
    ...
    if let Some(components) = value.get("components") {
        OpenApiContextGuard::with_context_mut(|ctx| {
            // Bolt: Performance optimization
            // Utilizing T::deserialize(val) to skip costly from_value(val.clone()) O(N) deep-cloning
            // allocations. Avoiding cloning nested components drastically improves parse time.
